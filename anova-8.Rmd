...
data <- read.csv("~/Box Sync/5067 Regression Spring/Spring_2019/Anxiety.data.csv")
...
'3. Planned contrasts/Post hoc comparisons/simple main effects
 
 - All of these fall under the rubric of 'contrasts'  
 - The most basic o these is Tukey honestly significant difference
...
em.1<- emmeans(aov.1, c("group", "SupportLevel"))
...
## Quick plot
```{r}
emmip(aov.1, group ~ SupportLevel)
```
...
# lsmeans but it has one major downside: MASS is loaded and 
...
```{r, message=FALSE}
library(phia)
testInteractions(aov.1, pairwise="SupportLevel")
```

##
```{r}
testInteractions(aov.1, pairwise="group")
```
