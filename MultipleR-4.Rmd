...
```{r}
library(rgl)
newdat <- expand.grid(Support=seq(0,18,by=1),Anxiety=seq(0,15,by=1))
newdat$pp <- predict(mr.model,newdat=newdat)
with(Multipleregression, plot3d (Support, Anxiety, Stress, size = 8, col="blue"), add = T)
with(newdat,surface3d(unique(Support),unique(Anxiety), pp, alpha=0.3,front = "line", back = "line"))
```
...
```{r, echo = FALSE}
one.way <- read.csv("~/Box Sync/5067 Regression Spring/anova.csv")
